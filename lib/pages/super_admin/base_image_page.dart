...
class _BaseImagePageState extends State<BaseImagePage> {
  ...
  void initState() {
    super.initState();
    context.read<BaseImageBloc>().add(const GetAllBaseImages());
  }
    ...
    return Scaffold(
      ...
      body: BlocBuilder<BaseImageBloc, BaseImageState>(
        builder: (context, state) {
          if (state is BaseImageLoading) {
            return const AppLoader();
          }
          if (state is BaseImageError) {
            return AppError(msg: state.error!.message ?? 'Api Error');
          }
          if (state is BaseImageDone) {
